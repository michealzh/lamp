...
badcount=3
print_usage() {
	echo "USAGE: $0 [-w warn -c crit -e err_code -m -d /path/to/touchdir ] -p partition"
	echo '       -d Optional: /path/to/dir where the checked fs name should be touched.Note : Dont specify the railing slash in dir name.'
...
partition=dummydsk
while getopts d:w:c:e:p:mh options; do
	case ${options} in
	w) warn=$OPTARG;;
	c) crit=$OPTARG;;
	e) err_code=$OPTARG;;
...
if [[ $partition == 'dummydsk' ]];then
       print_usage 
fi


if [[ -z $dir ]]; then
        disk_log='/usr/local/nagios/tmp/disk_checks'
else
        disk_log=$dir
fi

mkdir -p $disk_log # create dir if not exist.

##################################
part=$(echo $partition|sed 's/\//-sl-/g')
touch $disk_log/$part 2> /dev/null
##################################
check_process
...
if [[ $fields  -lt 5 || $fields -gt 6 ]]; then
        echo "BROKEN MONITOR::ESM00016::partition $partition got $fields fields | result=-1"
        exit 3
fi
### Get values ###
if [[ $fs = "local" ]]; then
...
#for i in `ls /usr/local/nagios/tmp/disk_checks`; do mv /usr/local/nagios/tmp/disk_checks/$i /usr/local/nagios/tmp/disk_checks/SL_$i;done

...
if [[ ($crit != "null" && $used_p -ge $crit) ]]; then
           /bin/echo "Memory CRITICAL::$err_code::warn=$warn,critical=$crit::used=$used_p%,free=$free_p%,total=$total_mb mb | used=$used_mb, used_p=$used_p, free_p=$free_p, total=$total_mb"
           exit 2
elif [[ ($warn != "null" && $used_p -ge $warn) ]];then
           /bin/echo "Memory WARNING::$err_code::warn=$warn,critical=$crit::used=$used_p%,free=$free_p%,total=$total_mb mb | used=$used_mb, used_p=$used_p, free_p=$free_p, total=$total_mb"
           exit 1
else 
           /bin/echo "Memory OK::$err_code::warn=$warn,critical=$crit::used=$used_p%,free=$free_p%,total=$total_mb mb | used=$used_mb, used_p=$used_p, free_p=$free_p, total=$total_mb" 
           exit 0
fi
